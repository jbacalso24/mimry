...
PLANNED_ADAPTERS: tuple[AdapterInfo, ...] = (
    ...
    AdapterInfo(
        ...
        status="planned",
        kind="data",
        extensions=("package.json", "pyproject.toml", "tsconfig.json", "vite.config.ts", "next.config.ts", "CLAUDE.md", "AGENTS.md"),
        parser="manifest-specific parsers",
        emits=("commands", "dependencies", "runtime_config", "agent_rules"),
        agent_use="Gives coding agents exact commands, package managers, frameworks, and repo rules before edits.",
        notes="High leverage for agent onboarding and verification planning.",
    ),
    AdapterInfo(
        name="swift-ios",
        status="planned",
