...
- Dependency policy: PEP 508 Git URL pinned to that commit in published MIMRY metadata
...
The upstream source is tracked as an optional git submodule at `vendor/graphify` instead of a copied tree so the MIMRY repo stays light while retaining a pinned, inspectable Graphify baseline. Normal installations use the same commit pin from package metadata; the submodule is not included in release artifacts.
