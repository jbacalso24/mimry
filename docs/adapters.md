...
| `js-ts-regex` | Regex import/export scan | `.js`, `.jsx`, `.ts`, `.tsx` | files, imports, exports, defines edges | Basic frontend routing until true TS/TSX AST exists |
...
1. `typescript-ast` — TS/TSX/JS/JSX symbols, React components, hooks, imports, exports, line ranges.
...
5. `config-manifest` — package/pyproject/tsconfig/vite/next/agent rule commands and dependency facts.
6. `sql-alembic` — schema tables, migrations, migration lineage, and DB edit gates.
7. `markdown-docs` — headings, links, specs, plans, decisions, Obsidian-style links later.
8. `swift-ios` — native iOS targets, entitlements, share extensions, App Groups.
...
- React screen/component question → prefer future `typescript-ast` + `nextjs-app-router` or `react-native-expo`.
